...
declare global {
  namespace JSX {
    interface IntrinsicElements {
      "golf-renderer": React.DetailedHTMLProps<IGolfProps, IGolfRenderer>;
    }
  }
}

export interface IGolfProps extends React.HTMLAttributes<IGolfRenderer> {
...
export interface IGolfRendererCommands {
  ...
  /**
   * Configure the colors and functionality of the renderer
   */
...
export interface RendererConfig {
  ...
  gradient_color?: [number, number, number];
  model_color?: [number, number, number];
  hover_color?: [number, number, number];
  cam_sens?: number;
  zoom_sens?: number;
  inspection_on?: boolean;
  watermark?: boolean;
}
